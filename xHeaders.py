...
async def GeT_Name(uid , Token):
    data = bytes.fromhex(await EnC_AEs(f"08{await EnC_Uid(uid , Tp = 'Uid')}1007"))
    ...
    if response.status_code == 200 or 201:
        ...
        BesTo_data = json.loads(await DeCode_PackEt(packet))      
        try:
...
async def DeLet_Uid(id , Tok):
    ...
    headers = {
        ...
        'Connection': 'Keep-Alive',
        'Accept-Encoding': 'gzip'}
    data = bytes.fromhex(await EnC_AEs(f"08a7c4839f1e10{await EnC_Uid(id , Tp = 'Uid')}"))
    ResPonse = requests.post(url , headers=headers , data=data , verify=False)    
